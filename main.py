...
try:
    ...
    app = Flask('')
    @app.route('/')
    def home(): return "Gladiador v12.4.1: Fix Running"
    def run_web(): app.run(host='0.0.0.0', port=8080)
except ImportError:
    app = None

# ==========================================
# 🔱 PARÁMETROS TÁCTICOS v12.4.1 (FIXED)
# ==========================================
NOMBRE_BOT = "GLADIADOR v12.4.1"
SIMBOLO = 'ETHUSDT'
PORCENTAJE_OP = 0.20   
LEVERAGE = 10          
ADX_ENTRADA = 22.0     
...
def obtener_datos():
    try:
        ...
        # CÁLCULO DE ADX - VERSIÓN CORREGIDA PARA NUMPY
        ...
        p_di_serie = 100 * (plus_dm.rolling(window=14).mean() / atr)
        m_di_serie = 100 * (minus_dm.rolling(window=14).mean() / atr)
        
        dx = 100 * abs(p_di_serie - m_di_serie) / (p_di_serie + m_di_serie)
        # ACÁ ESTABA EL ERROR: Primero calculamos la media de la serie, luego sacamos el último valor
        adx_serie = dx.rolling(window=14).mean()
        
        adx_val = adx_serie.iloc[-1]
        p_di = p_di_serie.iloc[-1]
        m_di = m_di_serie.iloc[-1]
...
def main_loop():
    global en_posicion, tipo_posicion
    print(f"🚀 {NOMBRE_BOT} - ATAQUE BI-DIRECCIONAL FIX")
        ...
        if p is not None:
            ...
            if not en_posicion:
                ...
                if tipo_posicion == 'LONG':
                    print(f"👀 [LONG] {fuerza_emoji} | P: {p:.1f} | ADX: {adx:.1f}")
                    if adx < umbral_salida or p < (ema * 0.998) or (m_di > p_di and m_di > 30):
                        cerrar_posicion("Salida de Long")
                elif tipo_posicion == 'SHORT':
                    print(f"👀 [SHORT] {fuerza_emoji} | P: {p:.1f} | ADX: {adx:.1f}")
                    if adx < umbral_salida or p > (ema * 1.002) or (p_di > m_di and p_di > 30):
                        cerrar_posicion("Salida de Short")
            sys.stdout.flush()
        time.sleep(30)
